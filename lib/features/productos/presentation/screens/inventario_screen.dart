...
class _InventarioScreenState extends ConsumerState<InventarioScreen> {
  ...
  // ejemplo, solo tocar un producto para seleccionarlo) volvía a recorrer
  // todo el catálogo con coincideFuzzy y a reordenarlo dentro de build(),
  // aunque nada de lo que afecta al filtro hubiera cambiado -eso era lo que
  // hacía sentir pesado el scroll con muchos productos cargados-.
  List<ProductoModel>? _productosCacheados;
  String? _vistaCacheada;
  ...
  // Ver el comentario junto a _productosCacheados: evita recorrer/reordenar
  // todo el catálogo en cada build si nada de lo que afecta el resultado
  // cambió desde la última vez.
  List<ProductoModel> _filtrarYOrdenar(
    List<ProductoModel> productos,
    String vista,
    String busqueda,
  ) {
    if (identical(productos, _productosCacheados) &&
        _vistaCacheada == vista &&
    ...
    if (busqueda.isNotEmpty) {
      lista = _busquedaPorCodigoBarras
          ? lista
                ...
                .toList()
          : lista
                .where((p) => coincideFuzzy(p.textoBusqueda, busqueda))
                .toList();
            ...
            child: NestedScrollView(
              ...
              body: Container(
                ...
                child: productosAsync.when(
                  data: (productos) {
                    final lista = _filtrarYOrdenar(productos, vista, busqueda);
                    _listaActual = lista;
